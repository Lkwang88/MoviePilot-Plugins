...
import Config from './__federation_expose_Config-28196ce33c.js';
...
async function onSave(payload) {
  try {
    await props.api.post('plugin/OguraSubscribePlus/config', payload);
    emit('action');
  } catch (err) {
...
onMounted(async () => {
  try {
    const response = await props.api.get('plugin/OguraSubscribePlus/config');
