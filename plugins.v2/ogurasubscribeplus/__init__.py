...
PLUGIN_ID = "OguraSubscribePlus"


class OguraSubscribePlus(_PluginBase):
    ...
    def __init__(self):
        # MP 运行时随后会调用 init_plugin；这里先建立安全的空状态，
        # 让插件方法在测试、API 探针和异常重载阶段也不会访问未定义属性。
        super().__init__()
        self._config = {}
        ...
        return [
            {
                ...
                "desc": "自定义识别词修正",
                "category": "小仓酱的订阅补全助手",
                "data": {"action": "ogurasubscribeplus_ci"},
            },
            {
                "cmd": "/sp",
                "event": EventType.PluginAction,
                "desc": "小仓酱的订阅补全助手待处理列表",
                "category": "小仓酱的订阅补全助手",
                "data": {"action": "ogurasubscribeplus_pending"},
            }
        ]
    ...
    def get_api(self) -> List[Dict[str, Any]]:
        return [
            {"path": "/status", "endpoint": self.get_status_api, "methods": ["GET"], "auth": "bear", "summary": "小仓酱的订阅补全助手状态"},
    ...
    def stop_service(self):
        ...
        try:
            ...
            logger.warning(f"小仓酱的订阅补全助手停止服务清理缓存失败：{exc}")
        self._scanner = None
        self._diagnoser = None
        self._site_resolver = None
    ...
    def save_config_api(self, payload: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            if not self.update_config(merged):
                return {"success": False, "message": "配置保存失败，未刷新插件和定时任务"}
            ...
            # 此配置页绕过宿主标准 PUT 接口，必须主动刷新当前插件的定时任务。
            # 否则 Cron/启用状态只会落盘，旧任务要等插件重载才会更新。
            try:
                from app.scheduler import Scheduler
                Scheduler().update_plugin_job(PLUGIN_ID)
            except Exception as exc:
                logger.error(f"小仓酱的订阅补全助手刷新定时任务失败：{exc}", exc_info=True)
                return {"success": False, "message": f"配置已保存，但定时任务刷新失败：{exc}"}
    ...
    def _fill_site_names(self, result: DiagnosisItem) -> None:
        ...
        try:
            ...
            logger.warning(f"小仓酱的订阅补全助手解析搜索站点名称失败：{exc}")
    ...
    def _diagnose_item_inner(self, item: DiagnosisInput) -> Optional[DiagnosisItem]:
        ...
        if mp_diagnosis.candidates:
            if mp_diagnosis.reason == "downloadable":
                logger.info(
                    "小仓酱的订阅补全助手触发 MP 订阅搜索后发现可匹配资源，已交给 MP 下载处理："
        ...
        logger.info(f"小仓酱的订阅补全助手：{item.title} 在 MP 订阅搜索范围内没有候选资源，不再执行插件 PT 范围兜底搜索")
        return None
        ...
        try:
            ...
            def wrapped_parse_result(
                ...
                try:
                    ...
                    logger.warning(f"小仓酱的订阅补全助手分析 MP 订阅搜索原始结果失败：{item.title}，{exc}")
            ...
            captured["errors"].append(str(exc))
            logger.warning(f"小仓酱的订阅补全助手触发 MP 订阅搜索失败：{item.title} ID={subscribe_id}，{exc}")
        return captured
    ...
    def _append_romaji_fallback_results(
        ...
        if not romaji_aliases:
            logger.info(f"小仓酱的订阅补全助手未找到可用罗马音别名：{self._format_item_log_context(item)}")
            return
        ...
        for alias in romaji_aliases:
            try:
                ...
                logger.warning(f"小仓酱的订阅补全助手罗马音补搜失败：{item.title}，关键词={alias}，{exc}")
    ...
    if eventmanager:
        ...
        def handle_plugin_action(self, event):
            ...
            if action == "ogurasubscribeplus_pending":
                self._handle_sp_command_text("/sp", event_data)
                return
            if action != "ogurasubscribeplus_ci":
                return
        ...
        def handle_transfer_complete(self, event):
            try:
                ...
                logger.warning(f"小仓酱的订阅补全助手入库后刷新诊断结果失败: {exc}")
    ...
    def _handle_callback(self, action: str, event_data: Dict[str, Any]):
        ...
        logger.info(f"小仓酱的订阅补全助手处理 Telegram 回调：{op}:{token}")
        if op == "close":
            self._ensure_store().delete_interaction(token)
            if token != "spmenu":
                self._notify_next_queued_show()
            if not self._delete_callback_message(event_data):
                self._post_callback_message(event_data, title="小仓酱的订阅补全助手", text="已关闭本次交互。", save_history=False)
            return
        state = self._ensure_store().load_interaction(token)
        if not state:
            self._post_callback_message(event_data, title="小仓酱的订阅补全助手", text="交互已过期，请重新扫描。", save_history=False)
            return
        ...
        if op == "ignore":
            ...
            self._ensure_store().delete_interaction(token)
            self._post_callback_message(event_data, title="小仓酱的订阅补全助手", text="已忽略本次提醒。", save_history=False)
            self._notify_next_queued_show()
            return
        if op == "back":
            self._post_callback_message(
    ...
    def _load_tmdb_target_summary(self, target: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.warning(f"小仓酱的订阅补全助手校验 TMDB 目标失败 TMDB={tmdbid}: {exc}")
    ...
    def _append_custom_identifiers(self, lines: List[str]) -> Dict[str, Any]:
        ...
        if added:
            ...
            try:
                ...
                logger.warning(f"小仓酱的订阅补全助手刷新识别词缓存失败: {exc}")
    ...
    def _start_moviepilot_subscribe_search(self, diagnosis: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            if callable(log_exception):
                log_exception("小仓酱的订阅补全助手触发 MP 原生订阅搜索失败")
            else:
                logger.warning(f"小仓酱的订阅补全助手触发 MP 原生订阅搜索失败: {exc}")
    ...
    def _download_candidate(self, diagnosis: Dict[str, Any], index: int, event_data: Optional[Dict[str, Any]] = None):
        ...
        if not (0 <= index < len(candidates)):
            self._post_callback_message(event_data, title="小仓酱的订阅补全助手", text="候选资源不存在。", save_history=False)
            return
        ...
        if not context:
            ...
            try:
                ...
                logger.warning(f"小仓酱的订阅补全助手读取候选缓存失败: {exc}")
        if not context:
            result = self._start_moviepilot_subscribe_search(diagnosis)
            ...
            self._post_callback_message(event_data, title="小仓酱的订阅补全助手", text=text, save_history=False)
            return
        try:
            ...
            self._post_callback_message(event_data, title="小仓酱的订阅补全助手", text=done_text, save_history=False)
        except Exception as exc:
            log_exception = getattr(logger, "exception", None)
            if callable(log_exception):
                log_exception("小仓酱的订阅补全助手提交候选资源下载失败")
            else:
                logger.warning(f"小仓酱的订阅补全助手提交候选资源下载失败: {exc}")
            self._post_callback_message(event_data, title="小仓酱的订阅补全助手", text=f"提交下载失败：{exc}", save_history=False)
    ...
    def _delete_callback_message(self, event_data: Dict[str, Any]) -> bool:
        try:
            ...
            logger.warning(f"小仓酱的订阅补全助手删除 Telegram 消息失败: {exc}")
            return False
    ...
    def _handle_transfer_complete_cleanup(self, event):
        ...
        if not match.matched:
            logger.info(
                f"小仓酱的订阅补全助手全集最终集处理跳过：{self._cleanup_history_label(current)}，原因={match.reason}"
            )
            return
        ...
        if mode != CLEANUP_OFF and not plan.should_cleanup:
            logger.info(
                f"小仓酱的订阅补全助手全集最终集清理跳过：{self._cleanup_history_label(current)}，原因={plan.reason}"
            )
        ...
        for history in plan.histories:
            try:
                ...
                logger.warning(f"小仓酱的订阅补全助手全集最终集清理失败：{self._cleanup_history_label(history)}，{exc}")
    ...
    def _ensure_season_pack_full_download(self, current, event_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            if len(files) <= 1:
                logger.info(
                    f"小仓酱的订阅补全助手跳过整季包全包下载：单文件种子非整季包，hash={download_hash}，files={len(files)}"
                )
                return {
                    "ok": False,
            ...
            logger.info(
                f"小仓酱的订阅补全助手已将整季包 qB 文件全部设为下载：hash={download_hash}，files={len(file_ids)}"
            )
            return {
                "ok": True,
            ...
            logger.warning(f"小仓酱的订阅补全助手设置整季包 qB 全包下载失败：hash={download_hash}，{exc}")
            return {
                "ok": False,
                "reason": str(exc),
                "file_count": 0,
    ...
    def _get_transfer_history_for_cleanup(self, history_id: int):
        try:
            ...
            logger.warning(f"小仓酱的订阅补全助手读取整理记录失败：{history_id}，{exc}")
            return None

    def _attach_cleanup_torrent_name(self, history, event_data: Dict[str, Any]):
        if not history or getattr(history, "_ogurasubscribeplus_torrent_name", None) or getattr(history, "torrent_name", None):
            return
        ...
        if not torrent_name:
            ...
            if download_hash:
                try:
                    ...
                    logger.warning(f"小仓酱的订阅补全助手读取下载历史种子名失败：hash={download_hash}，{exc}")
        if torrent_name:
            setattr(history, "_ogurasubscribeplus_torrent_name", torrent_name)
    ...
    def _load_transfer_histories_for_cleanup(self, current) -> List[Any]:
        try:
            ...
            logger.warning(f"小仓酱的订阅补全助手读取同季整理记录失败：{self._cleanup_history_label(current)}，{exc}")
            return []
    ...
    def _resolve_total_episode_for_cleanup(self, current, event_data: Dict[str, Any]) -> int:
        ...
        try:
            ...
            logger.warning(f"小仓酱的订阅补全助手读取订阅总集数失败：TMDB={tmdbid} S{season}，{exc}")
    ...
    def _resolve_subscribe_completed_for_cleanup(self, current, event_data: Dict[str, Any], total_episode: int) -> bool:
        ...
        try:
            ...
            logger.warning(f"小仓酱的订阅补全助手读取订阅完结状态失败：TMDB={tmdbid} S{season}，{exc}")
            return False
    ...
    def _delete_transfer_history_for_cleanup(self, history, delete_source: bool) -> bool:
        ...
        if delete_source and getattr(history, "src_fileitem", None):
            ...
            if not state:
                logger.warning(f"小仓酱的订阅补全助手删除源文件失败：{getattr(src_fileitem, 'path', '')}")
                return False
    ...
    def _refresh_scan_result_item(self, item: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            logger.warning(f"小仓酱的订阅补全助手复核已入库集失败: {exc}")
            return item
        if not downloaded:
            return item
    ...
    def _load_subscribes(self) -> List[Any]:
        try:
            ...
            logger.warning(f"小仓酱的订阅补全助手读取订阅失败: {exc}")
            return []
    ...
    def _get_subscribe(self, subscribe_id: int) -> Any:
        try:
            ...
            logger.warning(f"小仓酱的订阅补全助手读取订阅 {subscribe_id} 失败: {exc}")
            return None
    ...
    def _flatten_words(value: Any) -> List[str]:
        ...
        if isinstance(value, dict):
            ...
            for item in value.values():
                words.extend(OguraSubscribePlus._flatten_words(item))
            return words
    ...
    def _load_custom_release_groups(self) -> List[str]:
        ...
        try:
            ...
            logger.warning(f"小仓酱的订阅补全助手读取自定义制作组词表失败: {exc}")
    ...
    def _load_moviepilot_search_sites(self) -> List[Dict[str, Any]]:
        try:
            ...
            logger.warning(f"小仓酱的订阅补全助手读取搜索站点失败: {exc}")
            return []
    ...
    def _load_tv_categories(self) -> List[str]:
        try:
            ...
            logger.warning(f"小仓酱的订阅补全助手读取二级分类策略失败: {exc}")
            return []
    ...
    def _resolve_subscribe_category(self, subscribe: Any) -> Optional[str]:
        ...
        try:
            ...
            if category:
                ...
                return category
            logger.warning(f"小仓酱的订阅补全助手识别订阅分类未命中 {subscribe_label}")
        except Exception as exc:
            logger.warning(f"小仓酱的订阅补全助手识别订阅分类失败 {subscribe_label}: {exc}")
        return None
    ...
    def _load_tmdb_episodes(self, tmdbid: int, season: int, episode_group: Optional[str]) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            logger.warning(f"小仓酱的订阅补全助手读取 TMDB 剧集失败 TMDB={tmdbid} S{season}: {exc}")
            return []
    ...
    def _is_episode_downloaded(self, tmdbid: int, season: int, episode: int) -> tuple[bool, str]:
        try:
            ...
            logger.warning(f"小仓酱的订阅补全助手查询媒体库缓存失败: {exc}")
        ...
        try:
            ...
            logger.warning(f"小仓酱的订阅补全助手查询整理历史失败: {exc}")
    ...
    def _load_downloaded_episodes(self, tmdbid: int, season: int) -> set[int]:
        ...
        try:
            ...
            logger.warning(f"小仓酱的订阅补全助手读取媒体库已下载集失败: {exc}")

        try:
            history_dicts = self._load_transfer_history_dicts(tmdbid, season)
            episodes.update(episodes_in_transfer_history(history_dicts, tmdbid, season))
        except Exception as exc:
            logger.warning(f"小仓酱的订阅补全助手读取整理历史已下载集失败: {exc}")
    ...
    def _load_moviepilot_subscribe_sites(self, item: DiagnosisInput) -> List[str]:
        try:
            ...
            logger.warning(f"小仓酱的订阅补全助手读取 MP 订阅搜索站点失败：{item.title}，{exc}")
            return []
    ...
    def _search_torrents(self, item: DiagnosisInput, sites: Optional[List[str]] = None) -> List[Dict[str, Any]]:
        try:
            ...
            logger.warning(f"小仓酱的订阅补全助手搜索 PT 资源失败: {exc}")
            return []
    ...
    def _remember_download_context(self, context: Any, item: DiagnosisInput, title: str) -> str:
        ...
        try:
            ...
            logger.warning(f"小仓酱的订阅补全助手缓存候选下载信息失败: {exc}")
        return candidate_id
    ...
    def _rebuild_context_from_cache(self, cached: Dict[str, Any]) -> Any:
        ...
        try:
            ...
            logger.warning(f"小仓酱的订阅补全助手重建候选媒体信息失败，将按种子信息下载: {exc}")
