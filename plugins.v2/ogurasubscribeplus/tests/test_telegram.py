...
import ogurasubscribeplus as ogurasubscribeplus_module
from ogurasubscribeplus import OguraSubscribePlus
from ogurasubscribeplus.telegram import (
    build_ci_done_menu,
...
class TelegramTest(unittest.TestCase):
    def test_callback_uses_short_token_and_plugin_prefix(self):
        ...
        self.assertEqual(callback, "[PLUGIN]OguraSubscribePlus|download:abc123")
    ...
    def test_main_menu_has_snooze_3_days_button(self):
        ...
        self.assertIn("[PLUGIN]OguraSubscribePlus|snooze3d:show1", callbacks)

    def test_main_menu_has_manual_pt_scope_search_button_on_own_row(self):
        menu = build_main_menu("show1", True)

        self.assertIn({"text": "搜索其他站点", "callback_data": make_callback("ptscope", "show1")}, menu[1])
    ...
    def test_ci_command_is_registered(self):
        commands = OguraSubscribePlus.get_command()
        command = next(item for item in commands if item["cmd"] == "/ci")

        self.assertEqual(command["data"]["action"], "ogurasubscribeplus_ci")

    def test_sp_command_is_registered(self):
        commands = OguraSubscribePlus.get_command()
        command = next(item for item in commands if item["cmd"] == "/sp")

        self.assertEqual(command["data"]["action"], "ogurasubscribeplus_pending")
    ...
    def test_notification_title_uses_chinese_plugin_name(self):
        self.assertEqual(
            OguraSubscribePlus._notification_title({"title": "关于我转生变成史莱姆这档事"}),
            "小仓酱的订阅补全助手：关于我转生变成史莱姆这档事",
        )
    ...
    def test_close_callback_deletes_message_even_when_interaction_expired(self):
        ...
        plugin._handle_callback("[PLUGIN]OguraSubscribePlus|close:missing", {"original_message_id": 1, "original_chat_id": 2})
    ...
    def test_sp_menu_close_does_not_advance_notification_queue(self):
        ...
        plugin._handle_callback("[PLUGIN]OguraSubscribePlus|close:spmenu", {"original_message_id": 1, "original_chat_id": 2})
    ...
    def test_candidate_page_callback_renders_next_page(self):
        ...
        plugin._handle_callback("[PLUGIN]OguraSubscribePlus|cand2:tok", {})
    ...
    def test_download_callback_starts_moviepilot_subscribe_search(self):
        ...
        plugin._handle_callback("[PLUGIN]OguraSubscribePlus|download:tok", {})
    ...
    def test_start_moviepilot_subscribe_search_uses_scheduler_job(self):
        ...
        try:
            infos = []
            with patch.object(ogurasubscribeplus_module.logger, "info", side_effect=infos.append):
                result = OguraSubscribePlus()._start_moviepilot_subscribe_search(
    ...
    def test_candidate_download_logs_submitted_download_action(self):
        ...
        self.assertEqual(calls, [(plugin._download_contexts["c1"], "OguraSubscribePlus")])
    ...
    def test_pt_scope_callback_replaces_interaction_with_plugin_search_result(self):
        ...
        posts = []
        plugin._post_callback_message = lambda _event_data, **kwargs: posts.append(kwargs)
        plugin._manual_pt_scope_diagnosis = lambda diagnosis, only_sites=None: {
            **diagnosis,
            "reason": "rule_blocked",
        ...
        plugin._compute_other_sites = lambda _diagnosis: [
            {"id": "13", "name": "春天"},
            {"id": "20", "name": "观众"},
        ]

        plugin._handle_callback("[PLUGIN]OguraSubscribePlus|ptsall:tok", {})
        ...
        self.assertIn("搜索其他站点", [button["text"] for row in posts[-1]["buttons"] for button in row])
    ...
    def test_snooze_3_days_saves_until_and_advances_queue(self):
        ...
        plugin._handle_callback("[PLUGIN]OguraSubscribePlus|snooze3d:tok", {})
