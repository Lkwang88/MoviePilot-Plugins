...
from ogurasubscribeplus.season_cleanup import (
    CLEANUP_OFF,
    CLEANUP_RECORD,
    CLEANUP_SOURCE,
    build_cleanup_plan,
...
class SeasonPackCleanupTest(unittest.TestCase):
    ...
    def test_final_episode_uses_attached_download_torrent_name(self):
        ...
        setattr(current, "_ogurasubscribeplus_torrent_name", "Marriage Toxin S01 2026 1080p CR WEB-DL x264 AAC-Nest@ADWeb")
    ...
    def test_delete_transfer_history_uses_moviepilot_model_delete(self):
        ...
        with patch.dict("sys.modules", fake_modules):
            with patch.object(ogurasubscribeplus_module, "eventmanager", None):
    ...
    def test_delete_transfer_history_with_source_uses_moviepilot_source_cleanup(self):
        ...
        old_event_type = getattr(ogurasubscribeplus_module.EventType, "DownloadFileDeleted", None)
        setattr(ogurasubscribeplus_module.EventType, "DownloadFileDeleted", "download.file.deleted")
        try:
            with patch.dict("sys.modules", fake_modules):
                with patch.object(ogurasubscribeplus_module, "eventmanager", fake_eventmanager):
                    self.assertTrue(plugin._delete_transfer_history_for_cleanup(history(id=12), delete_source=True))
        finally:
            if old_event_type is None:
                delattr(ogurasubscribeplus_module.EventType, "DownloadFileDeleted")
            else:
                setattr(ogurasubscribeplus_module.EventType, "DownloadFileDeleted", old_event_type)
    ...
    def test_attach_cleanup_torrent_name_reads_download_history(self):
        ...
        self.assertEqual(
            getattr(current, "_ogurasubscribeplus_torrent_name"),
