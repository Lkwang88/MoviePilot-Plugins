...
from ogurasubscribeplus.storage import JsonStore


TEST_TMP_ROOT = Path.cwd() / ".codex_tmp_tests"


class IdentifierToolPlugin(OguraSubscribePlus):
...
class IdentifierApiTest(unittest.TestCase):
    ...
    def test_manual_identifier_adds_one_rule_and_records_result(self):
        ...
        self.assertEqual(len(plugin.appended), 2)
        self.assertTrue(plugin.appended[0].startswith("#"))
        self.assertIn("tmdbid=107371;type=tv", plugin.appended[1])
        self.assertFalse(plugin.appended[1].startswith("#"))
        record = plugin._ensure_store().load_identifier_records()[0]
        self.assertEqual(record["mode"], "force")
    ...
    def test_append_custom_identifiers_inserts_new_rule_at_top_and_clears_cache(self):
        ...
        self.assertEqual(
            result["added"], ["#注释不应写入", "New\\.Show => New{[tmdbid=2;type=tv]}"]
        )
        self.assertEqual(
