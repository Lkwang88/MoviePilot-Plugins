...
class SourceLayoutTest(unittest.TestCase):
    def test_diagnosis_cards_are_after_record_sections_in_dom(self):
        ...
        result_card_index = source.index('class="sp-inner-card mb-3"')

        self.assertLess(result_card_index, identifier_index)
        self.assertLess(result_card_index, rule_record_index)
